...
async function addAdmin() {
    ...
    if (args.length !== 1) {
        console.error('Uso: node add-admin.mjs <nickname>');
        process.exit(1);
    }
    ...
    try {
        ...
        if (user.admin) {
            console.log(`O usuário '${nickname}' já é um administrador.`);
            process.exit(0);
        }
        ...
        console.log(`Sucesso: '${nickname}' agora é um administrador!`);
        process.exit(0);
    } catch (error) {
