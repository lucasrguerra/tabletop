...
async function addFacilitator() {
    ...
    if (args.length !== 1) {
        console.error('Uso: node add-facilitator.mjs <nickname>');
        process.exit(1);
    }
    ...
    try {
        ...
        if (user.facilitator) {
            console.log(`O usuário '${nickname}' já é um facilitador.`);
            process.exit(0);
        }
        ...
        console.log(`Sucesso: '${nickname}' agora é um facilitador!`);
        process.exit(0);
    } catch (error) {
