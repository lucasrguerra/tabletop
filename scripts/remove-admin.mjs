...
async function removeAdmin() {
    ...
    if (args.length !== 1) {
        console.error('Uso: node remove-admin.mjs <nickname>');
        process.exit(1);
    }
    ...
    try {
        ...
        if (!user.admin) {
            console.log(`O usuário '${nickname}' já não é um administrador.`);
            process.exit(0);
        }
        ...
        console.log(`Sucesso: '${nickname}' não é mais um administrador!`);
        process.exit(0);
    } catch (error) {
